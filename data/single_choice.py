...
SINGLE_CHOICE_QUESTIONS = [
    ...
    },
    {
        ...
        "options": [
            "A. 三者都是 AWS 全球内容分发网络 (CDN) 的不同实现，核心功能完全一致，主要用于缓存静态内容",
    ...
    },
    {
        ...
        "options": [
            "A. Control Tower 完全替代了 Organizations，客户应直接使用 Control Tower 而非 Organizations",
            "B. Control Tower 构建在 Organizations 之上，自动配置 Organizations 结构，并额外提供预设的 Guardrails、蓝图和 Account Factory，使多账户治理更加自动化和标准化",
            "C. Organizations 提供多账户结构管理，Control Tower 则专注于安全组和网络 ACL 的集中管控",
            "D. 两者提供完全相同的多账户治理功能，只是 AWS 推出的不同产品名称",
        ],
        "correct_answers": [
            "B",
        ],
        "explanation": "「Control Tower 构建在 Organizations 之上，自动配置 Organizations 结构，并额外提供预设的 Guardrails、蓝图和 Account Factory...」是正确的。\n\nControl Tower 建立在 AWS Organizations 基础之上。它会自动为你创建并配置 Organizations（包括管理账户、日志归档账户和审计账户），并在此基础上叠加更高级的治理功能：预构建的 Guardrails（基于 SCP 和 Config 规则）、账户蓝图（Blueprint）和 Account Factory（自动化账户创建流程）。\n\n其他选项分析：\n\n「Control Tower 完全替代了 Organizations，客户应直接使用 Control Tower 而非 Organizations」是错误的：Control Tower 本身就是建立在 AWS Organizations 之上的，它会帮你自动创建和配置 Organizations，而非替代它。\n\n「Organizations 提供多账户结构管理，Control Tower 则专注于安全组和网络 ACL 的集中管控」是错误的：Control Tower 提供的核心价值是开箱即用的治理框架（Guardrails、Account Factory、蓝图），而非仅限于安全组管理。\n\n「两者提供完全相同的多账户治理功能，只是 AWS 推出的不同产品名称」是错误的：Organizations 提供基础的多账户结构和 SCP，Control Tower 在其上提供了更高级、更自动化的治理能力，两者定位不同。\n\n**重点考点 / 关键词补充：**\n- Control Tower 构建于 Organizations 之上\n- 额外价值：预设 Guardrails + Account Factory + 蓝图\n- 常考：Control Tower 让多账户治理“开箱即用”，大幅降低手动配置 Organizations + SCP + Config 的工作量",
        "domain": "Security and Compliance",
    },
    {
        "id": "S139",
