...
MULTI_CHOICE_QUESTIONS = [
    ...
    },
    {
        ...
        "explanation": "正确答案：\n\n「当前 Spot 价格与 On-Demand 价格的差距」\n「所选实例类型和可用区的供需情况」\n\n错误选项分析：\n\n「当前实例已经连续运行了多长时间（运行越久越容易被中断）」是错误的：Spot 实例被中断的概率主要由当前供需关系和价差决定，与已经运行了多长时间几乎没有关系。\n\n「是否开启了 CloudTrail 日志记录和审计跟踪功能」是错误的：CloudTrail 只负责 API 调用审计，与 Spot 实例的供需中断机制完全无关。\n\n「是否为该 Spot 实例附加了 IAM Role 或执行角色」是错误的：IAM Role 只影响权限边界，不参与 AWS 内部的 Spot 容量供需和中断决策。\n\n**重点考点 / 关键词补充：**\n- **Spot Instance**：使用 AWS 富余容量，价格可比 On-Demand 低 90%，但可能被中断（2 分钟通知）。\n- **中断概率主要影响因素**：当前 Spot 价格与 On-Demand 的价差 + 所选实例类型和 AZ 的供需关系。\n- 价差越大（越便宜），被中断概率越高；热门实例类型（如特定 GPU 实例）也更容易被中断。\n- 常考：不是所有工作负载都适合 Spot，要做好中断处理（Checkpointing、Queue 等）。",
        "domain": "Technology and Services",
    },
    {
        "id": "M23",
    ...
    },
    {
        ...
        "correct_answers": ["A", "B"],
        "explanation": "正确答案：\n\n「数据主权与合规要求」\n「目标用户地理位置与网络延迟」\n\n错误选项分析：\n\n「所有 Region 服务、定价、功能完全相同」是错误的：服务可用性、价格、合规认证在不同 Region 可能不同。\n\n「仅由开发团队技术偏好决定」是错误的：Region 选择是业务+合规+成本的综合决策。\n\n**重点考点 / 关键词补充：**\n- Region 选择四要素：合规、延迟、服务可用性、定价差异\n- 部分新 Region 可能只有 2 个 AZ\n- 多 Region 架构用于灾备，不是日常默认",
        "domain": "Cloud Concepts",
    },
    {
        "id": "M125",
    ...
    },
    {
        ...
        "correct_answers": ["A", "B"],
        "explanation": "正确答案：\n\n「GuardDuty 持续检测可疑活动与异常 API 行为」\n「Security Hub 聚合多服务发现，提供统一合规视图」\n\n错误选项分析：\n\n「Pricing Calculator 估算 SOC 人力成本」是错误的：Pricing Calculator 做费用预估，不做安全运营。\n\n「Glacier Deep Archive 拦截 SQL 注入」是错误的：Glacier 是归档存储，WAF 才防护 Web 攻击。\n\n**重点考点 / 关键词补充：**\n- 检测：GuardDuty、Macie、Inspector\n- 聚合：Security Hub\n- 防护：WAF、Shield、Network Firewall",
        "domain": "Security and Compliance",
    },
    {
        "id": "M131",
    ...
    },
    {
        ...
        "correct_answers": ["A", "B"],
        "explanation": "正确答案：\n\n「使用托管服务减少运维负担，聚焦业务性能」\n「根据工作负载选择合适资源并持续监控调优」\n\n错误选项分析：\n\n「统一最大规格 EC2 避免性能测试」是错误的：过度配置浪费成本且违反性能效率原则。\n\n「禁用 CloudWatch 指标」是错误的：可观测性是性能调优基础，且指标开销通常很小。\n\n**重点考点 / 关键词补充：**\n- 性能效率：正确资源选型 + 监控 + 托管服务\n- vs 成本优化：性能效率关注「用对资源」，成本优化关注「花少钱」\n- 常考：Right-sizing 是共享实践",
        "domain": "Cloud Concepts",
    },
    {
        "id": "M134",
        ...
        "correct_answers": ["A", "B"],
        "explanation": "正确答案：\n\n「Security Group 有状态，返回流量通常自动允许」\n「NACL 无状态，需同时配置入站和出站规则」\n\n错误选项分析：\n\n「两者都只支持 Allow」是错误的：NACL 支持显式 Deny；Security Group 仅支持 Allow（默认拒绝）。\n\n「NACL 仅作用于单实例、SG 作用于子网」是错误的：恰恰相反——SG 作用于实例/ENI，NACL 作用于子网。\n\n**重点考点 / 关键词补充：**\n- SG：有状态、实例级、仅 Allow\n- NACL：无状态、子网级、Allow + Deny\n- CLF 最高频网络对比考点之一",
        "domain": "Security and Compliance",
    },
    {
        "id": "M135",
        ...
        "correct_answers": ["A", "B"],
        "explanation": "正确答案：\n\n「启用 SSE 保护静态数据」\n「Block Public Access + 最小权限桶策略防公开」\n\n错误选项分析：\n\n「关闭访问日志和 CloudTrail 提升安全」是错误的：审计日志是安全检测与取证的基础。\n\n「所有桶公共可读以获得免费 DDoS」是错误的：公开桶是严重安全风险；CloudFront 不等于可公开源站。\n\n**重点考点 / 关键词补充：**\n- S3 安全三板斧：加密 + 访问控制 + 审计日志\n- SSE-KMS 可提供更细粒度密钥审计\n- Macie 可补充敏感数据发现",
        "domain": "Security and Compliance",
    },
    {
        "id": "M136",
