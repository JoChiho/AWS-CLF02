...
> 275 道高质量题目（155 单选 + 120 多选）+ 领域分类练习 + 模拟考试 + 完整持久化进度追踪

---

## ✨ 功能亮点

- **275 道精选题目**（155 单选 + 120 多选），全部配有详细解析和「重点考点」补充
- **模拟考试模式**（65 题 / 90 分钟 / 按官方领域权重抽题 / 严格无解析 / 70% 及格线）
- **练习模式中文术语标注**：英文 AWS 服务名显示为 `Spot Instances（竞价实例）`；模拟考试保持纯英文
...
- **总规模**：275 题（155 单选 + 120 多选）
- **ID 范围**：单选 S01–S160，多选 M01–M120
- **领域分布**：

| 领域 | 单选 | 多选 | 总计 | 占比 |
|------|------|------|------|------|
| Cloud Concepts（云概念） | 26 | 27 | 53 | 19.3% |
| Security and Compliance（安全与合规） | 43 | 31 | 74 | 26.9% |
| Technology and Services（技术与服务） | 61 | 36 | 97 | 35.3% |
| Billing, Pricing, and Support（账单定价与支持） | 25 | 26 | 51 | 18.5% |
| **总计** | **155** | **120** | **275** | 100% |

- **质量状态**：选项长度审计 **0 问题**；干扰项 High/Medium **0 道**
- **特色文档**：
...
- **练习模式**：单选点击即显示解析；多选需「提交答案」；英文术语自动附中文标注
...
│   ├── menu.py             # 主菜单
│   ├── quiz_view.py        # 答题界面（练习模式术语标注）
│   ├── stats_view.py       # 历史 / 统计
│   ├── mock_exam.py        # 模拟考试 UI
│   ├── wrong_book_view.py  # 增强错题本
│   ├── term_glossary.py    # AWS 英文术语 → 中文标注
...
├── data/
│   ├── __init__.py         # 数据层统一出口
│   ├── single_choice.py    # 155 道单选题
│   ├── multi_choice.py     # 120 道多选题
...
- 历史记录、错题统计保存在 `user_data.json`（已 `.gitignore`，仅保留在本地）
- 可随意备份、复制到其他机器继续使用
- 删除该文件即可清空所有进度
