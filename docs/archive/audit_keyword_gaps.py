...
def load_corpus():
    corpus_path = Path("docs/question_bank_corpus.txt")
    if not corpus_path.exists():
        # Rebuild on the fly
        print("Corpus not found, rebuilding...")
        single = load_questions_text("data/single_choice.py", "SINGLE_CHOICE_QUESTIONS")
        multi = load_questions_text("data/multi_choice.py", "MULTI_CHOICE_QUESTIONS")
...
def main():
    ...
    out_path = Path("docs/keyword_coverage_gap_report.txt")
    with out_path.open("w", encoding="utf-8") as f:
        f.write("=" * 70 + "\n")
        f.write("AWS CLF-C02 题库关键词覆盖差距审计报告\n")
        f.write("基于 docs/AWS-CLF-C02_All_Knowledge_Points.md vs 当前题库 (210题)\n")
        f.write("=" * 70 + "\n\n")
